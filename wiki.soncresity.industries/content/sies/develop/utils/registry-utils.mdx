...
<Callout title="Warning" type="warn">
  An error was made in versions below 2.3.0 where you could only use the RegistryUtils to register items and blocks to SI: Essentials itself and not to your own mod.
  This has been fixed in Version 2.3.0 by introducing a new `modid` argument to each Method.
</Callout>
...
```
```java tab="sies <=2.2.2-fabric & >=2.2.0-fabric"
